...
func NewConnectionParams(netIP net.IP, port int, privateKey *ecdsa.PrivateKey, notBefore time.Time, mode ServerMode) *ConnectionParams {
...
func (cp *ConnectionParams) FromString(s string) error {
	...
	cp.notBefore = t
	cp.serverMode = ServerMode(new(big.Int).SetBytes(base58.Decode(sData[5])).Int64())
